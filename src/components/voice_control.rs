...
pub fn VoiceControl(
    ...
    view! {
        <div class="flex flex-col items-center p-4 border border-black border-solid rounded-sm">
            <p class:text-red-600=voice_state
                .mute_playback_signal(any_voice_solo)>{voice_state.name.clone()}</p>
            <div class="flex flex-row space-x-1">
                <button
                    class="border border-black rounded-sm px-1"
                    class:bg-red-600=voice_state.mute
                    on:click=move |_| toggle_and_disable(voice_state.mute, voice_state.solo)
                >
                    Mute
                </button>
                <button
                    class="border border-black rounded-sm px-1"
                    class:bg-sky-600=voice_state.solo
                    on:click=move |_| toggle_and_disable(voice_state.solo, voice_state.mute)
                >
                    Solo
                </button>
            </div>
